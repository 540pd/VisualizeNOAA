...
eq_location_clean() %\>% filter(country==“MEXICO” &
lubridate::year(date) \>= 2000) %\>% eq_map(annot_col = “date”)
...
interacts with the map. eq_clean_data(“../inst/extdata/earthquakes.tsv”)
%\>% eq_location_clean() %\>% filter(country==“MEXICO” &
lubridate::year(date) \>= 2000) %\>% mutate(popup_text =
